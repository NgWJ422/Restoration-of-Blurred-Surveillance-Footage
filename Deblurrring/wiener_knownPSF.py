...
def main():
    '''
    Without the denoise, histogram equalized and sharpening
    Performance:
    - Gaussian: good
    ...
    With the denoise, histogram equalized and sharpening
    Performance:
    - Gaussian: good, but has artifacts
    - directional: good, but has artifacts
    - bilateral: not implemented
    - median: not implemented
    - box: ok but has some artifacts
    - motion: good, but has artifacts
    '''
    # Ask for the file path
    file_path = "../pic_manually_blurred/motion/hexagon_building_1/" 
    ...
    if blur_type in ['motion', 'directional']:
        kernel = params
        deblurred_image = deblur_image(imageGray_eq, kernel)
    else:
